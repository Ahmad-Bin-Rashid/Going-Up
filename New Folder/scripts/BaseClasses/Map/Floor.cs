...
namespace GoingDown
{
    public class Floor
    {
        ...
        private void GenerateMap(int minRooms, int maxRooms)
        {
            ...
            spawn = new Room(Utils.RoomType.Spawn, (0, 0));
            boss = new Room(Utils.RoomType.Boss, (100, 100));
            Rooms.Add(spawn); // Spawn room at (0,0)
            Rooms.Add(boss); // Boss room at (10,10)

            // Step 2: Ensure at least one of each required room type
            Rooms.Add(new Room(Utils.RoomType.Large, (random.Next(1, 100), random.Next(1, 100))));
            Rooms.Add(new Room(Utils.RoomType.Large, (random.Next(1, 100), random.Next(1, 100))));
            ...
            for (int i = 0; i < roomCount - 4; i++) // Remaining rooms after the fixed ones
            {
                ...
                Utils.RoomType randomType = (Utils.RoomType)random.Next(2, 5); // Random between Large, Treasure, Trap
        ...
        private void ConnectRooms()
        {
            foreach (var room in Rooms)
            {
                ...
                int minConnections = room.Type == Utils.RoomType.Spawn || room.Type == Utils.RoomType.Boss ? 1 : 2;
                int maxConnections = room.Type == Utils.RoomType.Spawn || room.Type == Utils.RoomType.Boss ? 1 : 3;
